...
export default function Home() {
  ...
  useEffect(() => {
    // Mengecek sesi login saat halaman dimuat
    const session = localStorage.getItem('matchUserSession');
    const name = localStorage.getItem('matchUserName');
    if (session === 'active') {
      setIsLoggedIn(true);
  ...
  const handleLogout = () => {
    // Menghapus sesi saat klik logout
    localStorage.removeItem('matchUserSession');
    localStorage.removeItem('matchUserName');
    setIsLoggedIn(false);
  };
    ...
    <div className="min-h-screen bg-[#050b14] relative overflow-x-hidden font-sans text-slate-200">
      ...
      <nav className="fixed top-0 w-full p-4 md:p-6 flex justify-between items-center z-50 border-b border-cyan-500/20 bg-[#0a1120]/80 backdrop-blur-xl shadow-[0_4px_30px_rgba(34,211,238,0.05)]">
        ...
        <div className="flex justify-between items-center max-w-7xl mx-auto w-full px-2">
          ...
          {/* MENU KANAN DINAMIS (Berdasarkan Status Login) */}
          <div className="flex items-center gap-4 md:gap-6">
            ...
            {!isLoggedIn ? (
              ...
              <>
                ...
                <Link href="/dashboard" className="text-[10px] md:text-xs font-extrabold text-slate-900 bg-gradient-to-r from-cyan-400 to-violet-500 hover:from-cyan-300 hover:to-violet-400 px-5 py-2 md:py-2.5 rounded-full shadow-[0_0_20px_rgba(34,211,238,0.4)] transition-all flex items-center gap-2 hover:scale-105 uppercase tracking-widest">
                  Daftar Gratis
                </Link>
              </>
            ) : (
              ...
              <>
                <div className="hidden sm:block text-[10px] text-cyan-400 font-bold uppercase tracking-widest border-r border-slate-700 pr-4">
                  Halo, {userName}
                </div>
                <Link href="/dashboard" className="text-[10px] md:text-xs font-bold text-slate-300 hover:text-white transition-colors uppercase tracking-widest">
                  Dashboard
                </Link>
                <button onClick={handleLogout} className="text-[10px] md:text-xs font-extrabold text-slate-300 hover:text-rose-400 bg-rose-500/10 border border-rose-500/20 px-5 py-2 md:py-2.5 rounded-full transition-all uppercase tracking-widest">
                  Keluar
                </button>
              </>
            )}
        ...
        <div className="flex flex-col sm:flex-row items-center gap-6">
          <Link href="/dashboard" className="relative inline-flex group">
            ...
            <button className="relative inline-flex items-center justify-center px-10 py-4 text-base font-extrabold text-white transition-all duration-200 bg-[#050b14] font-pj rounded-full focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-cyan-500 border border-slate-800 tracking-wider">
              {isLoggedIn ? "BUKA DASHBOARD 🚀" : "MULAI ANALISIS SISTEM 🚀"}
            </button>
          </Link>
